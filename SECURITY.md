...
We take the security of NuGet Cache Tool seriously. If you believe you have found a
...
When using NuGet Cache Tool, we recommend following these security best practices:

### Input Validation

- Validate command-line arguments and file paths before processing
- Be cautious when processing data from untrusted sources
- Use the latest version of NuGet Cache Tool to benefit from security updates

### Dependencies

- Keep NuGet Cache Tool and its dependencies up to date
...
- Run NuGet Cache Tool with the minimum required permissions
- Avoid running NuGet Cache Tool as a privileged user unless necessary
...
NuGet Cache Tool reads and writes files on the local file system. Users should:
...
NuGet Cache Tool relies on third-party packages. We:
...
Thank you for helping keep NuGet Cache Tool and its users safe!

[security-advisories]: https://github.com/demaconsulting/NuGetCacheTool/security/advisories
